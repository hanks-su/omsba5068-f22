...
def test_ghost_probs():
    ...
    reportTest("Prob (4,6)", gp.coord_prob((4,6)), 0.027773534598880677)
    gp.update_probe(('orange', (6,5)))
    gp.update_probe(('red', (1,1)))
    gp.update_probe(('red', (1,2)))
    reportTest("Prob (2,4)", gp.coord_prob((2,4)), 0.004825217967733085)
    sp = gp.sorted_coords()
    reportTest("Most probable", sp[0][1], 0.3455775154033599)
    gp.update_bust((False, (3,10)))
    reportTest("Post false update (3,10)", gp.coord_prob((3,10)), 0.0)
    reportTest("Post false update (5,5)", gp.coord_prob((5,5)), 0.3455775352543943)
